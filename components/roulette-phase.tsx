...
export function RoulettePhase({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-8 animate-in fade-in duration-700">
      ...
      {/* MOLDURA EXTERNA (Visual Premium) */}
      ...
      {/* BOTÃO DE AÇÃO (Corrigido para Alto Contraste na TV) */}
      <button
        onClick={handleAction}
        disabled={isSpinning}
        autoFocus
        className={cn(
          "relative px-16 py-8 rounded-full font-black text-4xl uppercase tracking-widest transition-all",
          "border-[6px] border-transparent", // Borda transparente que fica colorida no foco
          "hover:scale-105 hover:-translate-y-1",
          // Foco TV: Borda Amarela Brilhante
          "focus-visible:scale-110 focus-visible:border-yellow-400 focus-visible:outline-none focus-visible:shadow-[0_0_40px_rgba(255,255,0,0.6)]",

          isSpinning
            ? "bg-gray-400 text-gray-200 cursor-not-allowed opacity-50"
            : isLocked
            ? // ESTADO BLOQUEADO: Fundo Branco Sólido + Texto Laranja (Legibilidade Máxima)
              "bg-white text-[#FF6B00] shadow-[0_10px_0_#e5e5e5]"
            : // ESTADO LIBERADO: Fundo Verde Sólido + Texto Branco
              "bg-green-600 text-white shadow-[0_10px_0_#14532d]"
        )}
      >
